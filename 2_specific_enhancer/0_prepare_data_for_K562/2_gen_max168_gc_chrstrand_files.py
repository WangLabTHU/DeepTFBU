...
if __name__ == '__main__':
    ...
    for TF_name in TF_list:
        ...
        PPM_temp = TF_ppm[TF_name]
        seq_part = []
        pos_regions = open('./data_dir/step2_K562_pos_'+TF_name+'_data.bed', 'w')
        data_file = open('./data_dir/K562_ChIP_' + TF_name + '.bed')
        gc_pos = []
        ...
        PPM_temp = TF_ppm[TF_name]
        seq_part = []
        neg_regions = open('./data_dir/step2_K562_neg_'+TF_name+'_data.bed', 'w')
        data_file = open('./data_dir/step1_K562_negative_' + TF_name + '.bed')
        for lines in data_file:
            line = lines.split()
            seq_orig_info = genes[line[0]][int(line[1]):int(line[2])]
            seq_orig = seq_orig_info.seq.upper()
