...
if __name__ == '__main__':
    os.makedirs('./step1_HepG2_specific_gc10_check', exist_ok=True)
    TF_list = ['GATA2']
    ...
    for TF_name in TF_list:
        ...
        out_file = open('./step1_HepG2_specific_gc10_check/' + TF_name + '.txt','w')
        selected_seq_list = []
        opti_iter_num = 300
        for f_num in range(opti_iter_num):
            file_num = opti_iter_num-1-f_num
            loss = open('./step0_HepG2_specific_GA_log/score_mask_' + TF_name +'_'+str(file_num)+'.txt')
            all_scores = open('./step0_HepG2_specific_GA_log/score_all_mask_' + TF_name +'_'+str(file_num)+'.txt')
            seqs = np.load('./step0_HepG2_specific_GA_log/seqs_mask_' + TF_name +'_'+str(file_num)+'.npy')
