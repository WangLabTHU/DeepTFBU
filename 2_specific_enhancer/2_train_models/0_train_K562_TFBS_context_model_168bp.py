...
class Seq2ScalarTraining:
    def __init__(self,TF_name):
        ...
        f = h5py.File('../0_prepare_data_for_K562/data_dir/step4_K562_motif_center_seq_mask_'+str(seq_len)+'_'+TF_name+'_data.h5', 'r')
        ...
        self.save_path = './step0_K562_model_path/'
        if self.gpu:
    ...
    def training(self):
        ...
        np.savetxt('./step0_K562_log/'+self.name+'_' + self.TF_name +'_real_expr_final.txt',real_expr)
        np.savetxt('./step0_K562_log/'+self.name+'_' + self.TF_name +'_predict_expr_final.txt',predict_expr)
        
        fpr, tpr, threshold = roc_curve(real_expr, predict_expr)
        roc_auc = auc(fpr, tpr)
        auc_log_file = open('./step0_K562_log/log_masked.txt','a')
        ...
        plt.savefig('./step0_K562_log/'+self.name+'_' + self.TF_name +'_predict_expr_final.png')
        plt.close()
        log_file = open('./step0_K562_log/result_log.txt','a')
        log_file.write(self.TF_name + '\t' + self.name + '\t' + str(roc_auc) + '\n')
        log_file.close()
        trainingLog = pd.DataFrame(trainingLog)
        trainingLog.to_csv('step0_K562_log/training_log' + 'test_' + self.name +'_' + self.TF_name + '.csv', index=False)


def main():
    os.makedirs('./step0_K562_model_path', exist_ok=True)
    os.makedirs('./step0_K562_log', exist_ok=True)
    TF_list = ['GATA2']
